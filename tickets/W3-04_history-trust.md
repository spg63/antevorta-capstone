...
- **S1.** History store, per (agent, partner, sample): partner's prediction at encounter time, agreement,
  certainty delta, round. Queryable (tidy dataframe or SQLite); exposes a correctness back-fill hook (W4-02
  calls it when labels arrive).
...
4. Store completeness: every encounter recorded exactly once; the back-fill hook flips correctness fields
